...
function UploadFile() {
	...
	var url =  "./en-ajax.php?act=upload"; // 接收上传文件的后台地址
	...
	xhr.onload = function(evt) { //请求完成
        ...
        try{
            ...
            if(json.code == 0){
				var jumpurl = "en-file.php?hash="+json.hash;
				if($("#ispwd").prop('checked') && $("#pwd").val()!=''){
					jumpurl+='&pwd='+$("#pwd").val();
				}
                show_msg('Upload successful!The total available:'+nt.toFixed(2)+'Seconds.Jumping to file view page...');
                setTimeout(function(){ window.location.href=jumpurl; }, 800);
            }else{
                show_msg(json.msg, 1);
            }
        }catch(e){
            show_msg('Upload failed, please try again later or contact webmaster', 1);
        }
	};
...
function progressFunction(evt) {
	...
	if(resttime==0)$(".progress-bar").html('In storage');
}
